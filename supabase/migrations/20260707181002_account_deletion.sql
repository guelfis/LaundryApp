...
BEGIN
  ...
  LOOP
    ...
    -- RULE 1: If user is an Admin of a shared building but no other admins exist -> BLOCK
    IF v_membership_record.household_role = 'admin'::household_role 
       AND v_other_house_members > 0 
       AND v_other_house_admins = 0 
    THEN
      RAISE EXCEPTION 'SOLE_BUILDING_ADMIN_ERROR';
    END IF;

    -- RULE 2: If user is an Admin of a shared apartment but no other apartment admins exist -> BLOCK
    IF v_membership_record.apartment_role = 'admin'::apartment_role 
       AND v_other_apt_members > 0 
       AND v_other_apt_admins = 0 
    THEN
  ...
  LOOP
    ...
    -- Recalculate remaining member footprints for isolation checks
    ...
    ELSE
      v_other_apt_members := 1; -- Fallback skip marker
    END IF;

    -- CASE B: User is the absolute last member of an apartment -> Delete Apartment
    -- (ON DELETE CASCADE handles all internal apartment_invitations, join_requests, bookings)
    ...
    -- (This triggers your native cascade tree, matching your delete_household behavior perfectly)
  ...
  -- Wipes all personal links matching this user ID across any remaining houses they shared
