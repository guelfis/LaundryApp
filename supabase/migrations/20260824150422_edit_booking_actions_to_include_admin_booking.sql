...
CREATE OR REPLACE FUNCTION public.book_laundry_slot(
  ...
  start_hour INT,
  end_hour INT,
  requested_status TEXT DEFAULT 'active' -- 1. Added dynamic parameter with default value
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    VALUES (target_apartment_id, auth.uid(), computed_start, computed_end, validated_status)
...
GRANT EXECUTE ON FUNCTION public.book_laundry_slot(UUID, DATE, INTEGER, INTEGER, TEXT) TO authenticated;
